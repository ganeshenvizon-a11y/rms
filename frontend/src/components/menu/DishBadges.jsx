...
export const FoodTypeBadge = ({ foodType, className = '' }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border text-[11px] font-semibold ${meta.bgClass} ${meta.textClass} ${className}`}
    >
...
export const SpiceLevelBadge = ({ spiceLevel, className = '' }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border border-saffron-600/30 bg-saffron-100/60 text-maroon-900 text-[11px] font-semibold ${className}`}
      title={meta.label}
    >
